...
# Modifed by Dave Musicant, at Carleton College, based on Brad's version 1.6
...
class Pixel(object):
    ...
    def __init__(self, red=0, green=0, blue=0, gray=None):
    ...
    def getGray(self):
        ...
        return round((self.__red + self.__green + self.__red)/3)
    ...
    def setGray(self, gray):
        """Set all three colors to gray value"""
        self.__red = gray
        self.__green = gray
        self.__blue = gray
...
class AbstractImage(object):
    ...
    def __init__(self,fname=None,data=[],imobj=None,height=0,width=0):
        ...
        if pilAvailable:
            ...
            self.setPixel2D = self.setPILPixel
            self.set_pixel = self.setPILPixel
            self.set_pixel_2D = self.setPILPixel
            self.getPixel = self.getPILPixel
            self.getPixel2D = self.getPILPixel
            self.get_pixel = self.getPILPixel
            self.get_pixel_2D = self.getPILPixel
            ...
            self.setPixel2D = self.setTkPixel
            self.set_pixel = self.setTkPixel
            self.set_pixel_2D = self.setTkPixel
            self.getPixel = self.getTkPixel
            self.getPixel2D = self.getTkPixel
            self.get_pixel = self.getTkPixel
            self.get_pixel_2D = self.getTkPixel
    ...
    def getPixel1D(self,loc):
        ...
        return self.getPixel(loc % self.width, loc // self.width)

    def setPixel1D(self,loc,color):
        self.setPixel(loc % self.width, loc // self.width, color)
